...
## Summary
The test to set Target interval discount must be between 0 and 100e3. If 0, then the market will not decay.  But it does not revert when using integer underflow.  1 call to the test to set Target interval discount must be between 0 and 100e3. If 0, then the market will not decay passes and does not revert using integer underflow when it never should.  Because the underflow sets the value to maximum for uint48.  The result is market created with several payout tokens.
