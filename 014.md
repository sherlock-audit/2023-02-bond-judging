...
## Summary
The test to set max discount from current price must be between 0 and 100e3 (100%) or fail.  But it does not fail when using integer underflow.  1 call out of 1 to 3 calls the test to set max discount from current price beyond between 0 and 100e3 (100%) passes and does not revert using integer underflow when it never should.
